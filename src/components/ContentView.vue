...
export default {
  ...
  methods: {
    async fetchJsonData() {
      if (!this.isLoggedIn) {
        this.error = "User is not logged in. Please log in to search for addresses.";
        return;
      }
      ...
      try {
        ...
        const response = await axios.get(`http://localhost:8000/places?address=${address}&type=${type}`, { headers });
